...
export class SelectorEvents {
  ...
  public async refresh(selected?: string): Promise<void> {
    ...
    const friendlyName = fields.find((f) => f.referenceName == this.fieldName).name;
    this.valueObservable.value = await this._getSelected();
        
    const fieldMapString = SDK.getConfiguration().witInputs.FieldMap;
    if (fieldMapString) {
      try {
  ...
  private _setSelected = async (value: string): Promise<void> => {
    ...
    const restDataRow = this.serviceData.data.find(e => e === value);
    // Allow use to remove the value
    if ((!restDataRow) && (value)) {
